...
public static class AdminTraceEndpoints
{
    ...
    private static async Task<Results<Ok<IEnumerable<AdminTraceSummary>>, ProblemHttpResult>> GetTraces(
        int? days,
        ...
        CancellationToken ct)
    {
        ...
        var rows = await conn.QueryAsync<AdminTraceSummary>(
            """
    ...
    private static async Task<Results<Ok<IEnumerable<AdminTraceEntry>>, ProblemHttpResult>> GetTraceDetail(
        Guid correlationId,
        NpgsqlDataSource dataSource,
        CancellationToken ct)
    {
        await using var conn = await dataSource.OpenConnectionAsync(ct);
        var rows = await conn.QueryAsync<AdminTraceEntry>(
            """
            SELECT id                                        AS Id,
